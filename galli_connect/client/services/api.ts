...
export const login = async (email: string, password: string, role: UserMode): Promise<User> => {
    console.log(`Attempting to log in user: ${email} as ${role}`);
    if (!email || !password || password.length < 1) { // Basic check
        throw new Error('Invalid credentials');
    }
    // Mimic API call
    await delay({}, 700);
    // In a real app, you'd validate credentials. Here, we just return a mock user.
    return {
        id: 'user-123',
        name: role === 'DRIVER' ? 'Ramesh Kumar' : 'Sita Sharma',
        email: email,
        role: role,
    };
};

export const signup = async (name: string, email: string, password: string, role: UserMode): Promise<User> => {
